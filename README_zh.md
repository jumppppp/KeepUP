...
**14 个表达式函数** + 三元运算符 + 配置体系重构。 [完整更新日志](https://github.com/jumppppp/KeepUP/releases)

| 类别 | 函数 |
|------|------|
| 数学 | `abs` `sqrt` `pow` `min` `max` |
| 三角 | `sin` `cos` *(参数为角度，`sin(90)=1.0`)* |
| 距离 | `distance` `distance3d` |
| 角度 | `angle` `angle3d` |
| 中点 | `centerX` `centerY` |
| 条件 | `? :` |

> AI 智能检测新增三层过滤，不动也不再误检。
...
**🧠 AI 智能检测（新！）**：不用手动选运动，打开摄像头直接开练——KeepUp 自动识别你在做什么运动。一次训练可批量检测多种运动，每个条目独立生成骨架回放视频。
...
KeepUp 的运动检测引擎使用**自定义表达式语言**描述运动阶段。模板是 JSON 文件——修改无需重新编译 App。

```json
// 深蹲下降阶段：双膝弯曲至约 90°
"expr": "min(leftKneeAngle, rightKneeAngle) < 100"

// 身体直线度（平板支撑）：肩-髋-踝三点一线
"expr": "abs(angle(肩中点, 髋中点, 踝中点) - 180) < 10"

// 比例自校准阈值
"expr": "wristDistance < shoulderWidth * 1.5"
```

**14 个函数**：`abs` `sqrt` `pow` `min` `max` `sin` `cos` `distance` `distance3d` `angle` `angle3d` `centerX` `centerY` + 三元运算符 `? :`
